...
export class TelegramChannel implements Channel {
  ...
  private readonly limiter = createLimiter(
    'telegram', SECURITY_LIMITS.channels.maxInboundMessagesPerMinute, 60_000,
  );
  ...
  private setupCommands(): void {
    this.bot.command('start', (ctx) =>
      ctx.reply('OpenWind is ready. Use /ask, /status, /entity, /upcoming, /insights, /teach, or /silence.'));
    this.bot.command('ask', async (ctx) => {
      if (!ctx.match) { await ctx.reply('Usage: /ask <question>'); return; }
      await this.handleAsk(ctx, ctx.match);
    });
    this.bot.command('status', (ctx) => this.handleStatus(ctx));
    this.bot.command('entity', async (ctx) => {
      if (!ctx.match) { await ctx.reply('Usage: /entity <name>'); return; }
    ...
    this.bot.command('teach', async (ctx) => {
      if (!ctx.match) { await ctx.reply('Usage: /teach <fact>'); return; }
  ...
  private async handleStatus(ctx: Context): Promise<void> {
    const { db } = this.deps;
    const lastRow = db.prepare('SELECT MAX(timestamp) AS ts FROM events').get() as { ts: string | null } | undefined;
    const lines = [
      'OpenWind Status',
  ...
  private async handleEntity(ctx: Context, rawName: string): Promise<void> {
    ...
    if (!entity) { await ctx.reply(`Entity "${name}" not found.`); return; }
  ...
  private async handleUpcoming(ctx: Context, hours: number): Promise<void> {
    ...
    const lines = [`Upcoming (next ${String(hours)}h):`, ...deadlines.map((e) => `  [${e.timestamp}] ${e.eventType}`)];
    await ctx.reply(lines.join('\n'));
  }

  private async handleInsights(ctx: Context): Promise<void> {
    const rows: unknown[] = this.deps.db.prepare(
      "SELECT content, created_at FROM observations WHERE type = 'insight' ORDER BY created_at DESC LIMIT 5",
    ).all();
    if (rows.length === 0) { await ctx.reply('No insights generated yet.'); return; }
