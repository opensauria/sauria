...
export function renderRolePills(
  node: AgentNode,
...
export function renderAutonomy(
  node: AgentNode,
  fireUpdate: FireUpdate,
): TemplateResult {
  ...
  return html`
    <div class="section">
      ...
      <input type="text" .value=${node.description || ''}
        @input=${(e: InputEvent) => fireUpdate({ description: (e.target as HTMLInputElement).value || undefined })}
  ...
  return html`
    <div class="section">
      ...
      <textarea .value=${node.instructions || ''}
        @input=${(e: InputEvent) => fireUpdate({ instructions: (e.target as HTMLTextAreaElement).value })}
...
export function renderLanguage(
  ...
  return html`
    <div class="section">
      ...
      <select .value=${lang}
        @change=${(e: Event) => fireFn('language-change', { value: (e.target as HTMLSelectElement).value })}>
        ${RESPONSE_LANGUAGES.map((l) => html`
          <option value=${l.code} ?selected=${lang === l.code}>${l.label}</option>
        `)}
      </select>
    </div>
  `;
...
export function renderBehavior(
  ...
  return html`
    <div class="section">
      ...
      ${renderToggle(t('canvas.proactive'), behavior.proactive === true, 'proactive', node, fireUpdate)}
      ${renderToggle(t('canvas.ownerResponse'), behavior.ownerResponse !== false, 'ownerResponse', node, fireUpdate)}
...
export function renderKpis(
  ...
  const items = [
    ...
    {
      value: kpis.avgResponseTimeMs > 0
        ? (kpis.avgResponseTimeMs / 1000).toFixed(1) + 's'
        : '--',
